...
function parseVariants(obj) {
  ...
  for (const p of init.properties) {
    ...
    if (ts.isObjectLiteralExpression(p.initializer)) {
      ...
      // `play` is a closure like `render`; only its PRESENCE is recorded, so
      // the story projection can mount the variant on a `DemoStage` that runs
      // it, and the shot runner knows to wait for `data-stapel-play="done"`.
      const play = propInitializer(p.initializer, "play") !== null;
...
export function renderStory(demo, demoImport, groupPrefix) {
  ...
  const played = demo.variants.some((v) => v.play === true);
  ...
  for (const v of demo.variants) {
    ...
    used.add(name);
    // A variant with a `play` step mounts on a DemoStage, which runs the step
    // after mount and stamps `data-stapel-play="done"` for the shot runner.
    L.push(
      v.play === true
        ? `export const ${name} = (): ReactElement => <DemoStage demo={demo} variant=${JSON.stringify(v.id)} />;`
...
// ── shot-log errors → the package's defects (demos:errors) ──────────────────
//
// The shot harness (`shoot.mjs`) records, per Ladle story id and viewport tag,
// every `pageerror` / console error, a `NOT_MOUNTED` for Ladle's "Story not
// found", and a `BLANK_RENDER` for a story that mounted and painted nothing.
// Its JSON is `{ "<storyId>": { "<tag>": ["<error>", …] } }`. A story that
// errors is a defect of the package that owns it — a visual review must not
// be the first place it is noticed.

/** Ladle's story-id prefix for a package group (`auth-react` → `auth-react--`). */
export function storyGroupPrefix(group) {
  return `${group.toLowerCase().replace(/[^a-z0-9]+/g, "-")}--`;
}

/**
 * The entries of a shot-errors log that belong to `group`, flattened to
 * `[{ story, tag, errors }]`, sorted. `errors` keeps the harness' own
 * markers (`NOT_MOUNTED:`, `BLANK_RENDER:`, `NAV:`) verbatim.
 */
export function storyErrorsForGroup(errorsJson, group) {
  const prefix = storyGroupPrefix(group);
  const out = [];
  for (const [story, byTag] of Object.entries(errorsJson ?? {})) {
    if (!story.startsWith(prefix)) continue;
    if (!byTag || typeof byTag !== "object") continue;
    for (const [tag, errors] of Object.entries(byTag)) {
      if (!Array.isArray(errors) || errors.length === 0) continue;
      out.push({ story, tag, errors: errors.map(String) });
    }
  }
  return out.sort((a, b) => a.story.localeCompare(b.story) || a.tag.localeCompare(b.tag));
}

/** One line per (story, tag), first error line only — the listing CI prints. */
export function renderStoryErrorsReport(entries) {
  return entries.map((e) => {
    const first = (e.errors[0] ?? "").split("\n")[0];
    return `    - ${e.story} [${e.tag}] ${first}`;
  });
}
