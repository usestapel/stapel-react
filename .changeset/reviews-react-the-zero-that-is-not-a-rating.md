...
- **A 401 is not an empty list.** All four stapel-reviews views are
  `IsAuthenticated`, including both reads, so a signed-out visitor to a public
  listing page gets 401 — and the empty state would tell exactly the people who
  have not signed up yet that a well-reviewed seller has never been reviewed.
  `signInRequired` is a named state on both read bags. Upstream ask (AllowAny
  on the two GETs) recorded in `contract-pins.json`.
...
Two contract corrections the pair hand-declares, both with the upstream ask
beside them: `GET /reviews` returns core's `AnchorPagination` envelope while
the schema declares `ReviewResponse[]` (the view instantiates its paginator
inside `get()` instead of declaring a `pagination_class`, so drf-spectacular
never sees it), and its `anchor`/`limit`/`direction` parameters are undeclared
for the same reason — while `target_type`/`target_key`/`include`, added in
0.2.2, are what made this pair generatable at all.
...
pair because stapel-reviews ships no `translations/`.
