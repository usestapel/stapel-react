...
Ships the pair's first security-settings default-skin components (`@stapel/auth-react/default`): `SessionsList`, `TotpManager`, `PasskeysManager`, `PasswordChangePanel`, `OAuthLinks`. `OAuthLinks` is backed by a real `/oauth/links/` list/link/unlink surface found in stapel-auth's schema while building this and newly wired into `AuthApi` (`useOAuthLinks`/`useLinkOAuth`/`useUnlinkOAuth`); its "Connect" action and `PasskeysManager`'s "Add" both take a thin host binding (`getAccessToken`/`webauthnCreate`) for the browser-side ceremony this pair cannot perform itself, same boundary as the existing WebAuthn TODO.
