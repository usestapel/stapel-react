 ...
 * module-scoped fallback key (`currencies.error.unknown`, an en string in
 * {@link currenciesI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "currencies.error.unknown");
}
