/**
 * The viewer's display currency — the one thing this read-only module
 ...
 *
 *  - signed in (an active `SessionManager`) → `scope: "user"`, which is wiped
 *    on logout with no opt-out. A shared computer must not show the next
 *    person the previous one's chosen currency.
 *  - a visitor → `scope: "app"` in `localStorage`, which survives a reload and
 *    is not encrypted. A display preference is not a secret, and a visitor has
 *    no session key to encrypt it with.
 *
 * Reads try the user scope first and fall back to the visitor one, so a
 * currency chosen before signing in is still in effect after.
 */
 ...
 * profile field) passes its own implementation to `createCurrenciesRuntime`
 * instead — the seam is the interface, not this function.
 */
export function createDisplayCurrencyStore(): DisplayCurrencyStore {
  return {
    async read() {
      if (signedIn()) {
...
export function __resetDisplayCurrencyStores(): void {
  userRepo = null;
  visitorRepo = null;
}
