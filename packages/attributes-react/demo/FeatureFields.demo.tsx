...
export default defineDemo({
  ...
  variants: {
    "text and numbers": {
      description:
        "header, string, int, float, bool — a caption with no label or asterisk, a code-point counter rather than a hard cap (the DOM counts UTF-16, the engine counts code points), a translated postfix on the float, and a header that is a caption rather than a question.",
      viewport: "phone",
      step: "answered",
      render: () => (
    ...
    "ref-select": {
      description:
        "Two vocabulary-backed fields over a mock client: Brand searches a level of `phone-models`, Model searches the level below narrowed by the brand's code. The options are not in the category schema — they cannot be (529 vendors, 14 962 models) — so the config carries a pointer and the terms arrive over a second wire, debounced and superseding.",
      viewport: "desktop",
