...
- f14dd79: Raise the skin bundle's budget 36 -> 37.5 KB, measured at 36.15.
...
- 30a065b: The card type scale outranks antd's runtime sheet instead of racing it.
...
- 967be42: The card title and the card price are one scale, declared once, from the
  brand's ladder.
...
- 7c9178a: Three things the reviewers measured on a live classified at 390 / 768 / 1024 /
...
{display:inline-block; max-width:100%}`, which scores (0,1,1) against a single
  ...
  **The two rails are rails.** `RELATED_CARD_WIDTH` is one fixed 220px instead of
  ...
  its price 24px up: measured on one live rail, six prices at 1733px and two at 1709. And the rail has EDGE CONTROLS — both always rendered, `hidden` while
...
- e2ee9c0: The canned questions fit what the listing is about, and every one of them opens with a greeting.
...
- 4d56823: The list card fills its text column, with the description's opening
...
- e426b2d: The list card reads its rating with the price, through a slot of its own
...
- 68d5906: The last hand-written error override is gone, because upstream fixed the text.
...
- 70ca317: The seventeen refusals this pair used to write are the module's own words now
  ...
  **One override survives, and it is deliberate.** `error.409.invalid_listing_transition`
  ...
  ships a catalogue of its own now, but merging it _here_ would give one refusal two
...
- ab0e7a7: listings: the reader's two actions are 44px again, the share menu is reachable on a desktop, a save claims only what it can claim, and the page takes its geometry from the host (D450, D455, D456, D457, §25)
...
- 000724c: listings-react: the dashboard's Delete is drawn only where there is a route behind it
...
- 119f326: `<ListingDetailPane headingLevel>` — the listing title can be the page's `h1`
...
- e41fceb: `<ListingDetailPane>`: the page edge, and where the seller sits on a phone.
...
- 3168ad9: The favourites empty-state hint (`listings.favorites.empty_hint`) said "tap
