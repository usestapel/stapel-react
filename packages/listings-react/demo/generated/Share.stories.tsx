// AUTO-GENERATED by scripts/gen-demos.mjs — do not edit. Drift gate: pnpm gen:demos:check
// CSF story projected from a defineDemo() source (frontend-guardrails §4.1).
import type { ReactElement } from "react";
import { renderDemoVariant } from "@stapel/showcase";
import demo from "../Share.demo.js";

export default { title: "listings-react / Share a listing" };

export const Default = (): ReactElement => renderDemoVariant(demo, "default");
Default.storyName = "default";
Default.parameters = { stapel: {"viewport":"phone","step":"trigger_on_photo"} };
export const Menu = (): ReactElement => renderDemoVariant(demo, "menu");
Menu.storyName = "menu";
Menu.parameters = { stapel: {"viewport":"desktop","step":"menu_channels"} };
