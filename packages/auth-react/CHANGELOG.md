...
- 0be8f68: The deployment says what kind of stand it is, and a login grant stops being a
...
- 553d805: auth: the pair admits the release where `OPTIONS` answers instead of dying
...
- 0757117: auth: the OAuth door says where the sign-up came from, and the enum stops being three Google flavours
...
- c5460df: auth: the pin moves to stapel-auth 0.34.2, and the pair's generated surface gains an attribution object, a host→brand read, and one new error key
...
- cd39061: `refreshHandoffWindowMs` and `readSessionHint` reach core's session manager
...
- f89998b: Host chrome opt-out for `AuthPanel` (owner ruling: pair visuals must never
