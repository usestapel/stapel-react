// AUTO-GENERATED by scripts/gen-demos.mjs — do not edit. Drift gate: pnpm gen:demos:check
// CSF story projected from a defineDemo() source (frontend-guardrails §4.1).
import type { ReactElement } from "react";
import { renderDemoVariant } from "@stapel/showcase";
import demo from "../AuthenticatorChangePanelSkin.demo.js";

export default { title: "auth-react / Authenticator change (default skin)" };

export const Default = (): ReactElement => renderDemoVariant(demo, "default");
Default.storyName = "default";
Default.parameters = { stapel: {"viewport":"phone","step":"idle"} };
export const Scheduled = (): ReactElement => renderDemoVariant(demo, "scheduled");
Scheduled.storyName = "scheduled";
Scheduled.parameters = { stapel: {"step":"pendingDelayed"} };
