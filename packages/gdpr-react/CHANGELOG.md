...
- 1f8df08: Regenerated against **stapel-gdpr 0.7.1**: the pin was two minors behind
...
/gdpr/api/v1/dsar`, the account-close route and the data-export-request
  ...
  **Not from this bump.** The two releases that motivated moving the pin off
  ...
  friends match specific error _codes_, not status classes, so the new
...
- 553d805: gdpr: a closure that revoked every session now hands back a way to reach it
