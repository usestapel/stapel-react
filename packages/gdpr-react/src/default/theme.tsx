...
export interface GdprSkinThemeProps {
  /**
...
/**
 * `<GdprSkinTheme/>` — the self-theming wrapper of
 * `@stapel/gdpr-react/default`. Standalone use is supported (wrap any
 * composition of the skin's parts once); the shipped surfaces
 * (`AccountClosurePanel`, `PendingDeletions`, `DataExportPanel`, `DsarForm`,
 * and the two admin tables) each already wrap themselves, and
 * nested antd `ConfigProvider`s merge, so composing them under one
 * `GdprSkinTheme` stays correct.
 */
export function GdprSkinTheme(props: GdprSkinThemeProps): ReactElement {
