...
- a4d53ee: Both chromes change arms at the TABLET rung, and a host can name its own edge.
...
- eef0b6b: The storefront header is pinned by default, and a strip of the host's chips can
...
- 9d21fc9: Both chromes place the route they render: the top on a PUSH, where you left it
  on a POP.
...
- 723b1bb: shell: the pinned header paints its own seam, and holds its own layer (D458)
  ...
  and the content under it snap to device pixels _independently_, so at 0.5px
...
- 723b1bb: shell: `headerScrollFlag` is a HYSTERESIS — two edges, not one (D459)
...
- 182a3b4: shell: `--stapel-header-height` is 64px on a desktop again, whatever chrome the phone wears (D449)
  ...
  `.stapel-public-shell` (0,1,0) inside `@media (min-width:1200px)` at _every_
...
- 1cdb4eb: shell: `<PublicShell>`'s header geometry is PUBLISHED, and pinning it is a prop
...
- 9f71da3: `dockRenders` is now exported from `@stapel/shell-react/default`.
