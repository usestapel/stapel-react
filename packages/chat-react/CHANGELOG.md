...
- 08527b0: Voice messages — the audio hole is closed, both ways.
...
- aab68ca: An inbox row says WHICH kind of attachment, not just "some".
...
- 925f4ec: Attachments: the contract stapel-chat has been sending for five minors finally
  has a reader.
...
- 3dc36ed: The inbox's faces wear the same disc as every other surface.
...
- 5c3c637: The desktop inbox's empty right pane is a pane, and the divider between the two
...
- df257aa: `<ConversationSplitPanel listWidth>`: the desktop inbox's list rail is a
...
- ae128b6: A row's subject line becomes a seam, and a thread can open on the question
...
- f650634: The composer says who is writing — a refusal for a visitor, a warning for a guest.
...
- dcac226: chat: report and block wait for the person they are about
...
- 0f69ea4: chat: the threads you left have a listing, and a way back
...
- a3e99c2: chat: a person can leave a conversation, and the screen says that is not a delete
  ...
  **405** — there was no way off a thread at all — and now answers **204**: the
  caller's participant row is stamped `left_at`, the thread drops off _their_
...
- 630b418: **An inbox line with no words says WHICH kind of no words it is.** Pinned to
...
- 5050615: chat: the inbox is searched by the server, and a row carries the line it draws
...
- 8710bf4: The thread list can be searched, and can show only what is unread.
