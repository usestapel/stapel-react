// AUTO-GENERATED by scripts/gen-demos.mjs — do not edit. Drift gate: pnpm gen:demos:check
// CSF story projected from a defineDemo() source (frontend-guardrails §4.1).
import type { ReactElement } from "react";
import { renderDemoVariant } from "@stapel/showcase";
import demo from "../WorkspaceSettingsSkin.demo.js";

export default { title: "workspaces-react / Workspace settings (default skin)" };

export const Default = (): ReactElement => renderDemoVariant(demo, "default");
Default.storyName = "default";
Default.parameters = { stapel: {"step":"ready"} };
export const DeleteRefused = (): ReactElement => renderDemoVariant(demo, "delete-refused");
DeleteRefused.storyName = "delete-refused";
DeleteRefused.parameters = { stapel: {"viewport":"phone","step":"delete-blocked"} };
