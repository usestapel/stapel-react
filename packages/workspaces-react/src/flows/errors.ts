 ...
 * module-scoped fallback key (`workspaces.error.unknown`, an en string in
 * {@link workspacesI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "workspaces.error.unknown");
}
