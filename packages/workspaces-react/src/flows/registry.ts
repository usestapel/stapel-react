/**
 * Zero-flow registry shim (slim wave §21/S3). stapel-workspaces annotates no
 * `@flow_step` yet — its backend `docs/flows.json` carries no `workspaces.*`
 ...
 */
export const WORKSPACES_FLOWS = {} as const;

/** Canonical flow ids present in flows.json (none yet — see above). */
export type WorkspacesFlowId = keyof typeof WORKSPACES_FLOWS;

export type WorkspacesFlowSpec = (typeof WORKSPACES_FLOWS)[WorkspacesFlowId];
...
export function flowEndpoints(id: WorkspacesFlowId): readonly FlowEndpoint[] {
  // Same widened body as the generated registry's — valid for the zero-flow
  // shape AND correct once flows exist.
  const spec = WORKSPACES_FLOWS[id] as
