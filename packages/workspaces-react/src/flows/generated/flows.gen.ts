...
/**
 * The canonical workspaces flow registry — the machine-facing projection of the
 ...
 * `WORKSPACES_FLOWS[...].id` so a backend flow rename/re-step surfaces as a drift
 * here (`pnpm gen:flows:check`) rather than a silent analytics/contract skew.
 */
export const WORKSPACES_FLOWS = {
...
export function flowEndpoints(id: WorkspacesFlowId): readonly FlowEndpoint[] {
  ...
  // `WorkspacesFlowId` = `never` and `WORKSPACES_FLOWS` = `{}`, so a direct
  // `WORKSPACES_FLOWS[id].steps` does not type-check. Widening to an optional spec
