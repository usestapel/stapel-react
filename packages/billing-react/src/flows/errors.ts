 ...
 * module-scoped fallback key (`billing.error.unknown`, an en string in
 * {@link billingI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "billing.error.unknown");
}
