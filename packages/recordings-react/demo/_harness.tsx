/**
 * Shared harness for the recordings-react demos (frontend-guardrails §4.2). Demos
 ...
 * headless component needs: query client, i18n, and the recordings runtime.
 */
...
import { RecordingsProvider, registerRecordingsI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-recordings `/recordings/api/`). */
export const DEMO_BASE = "https://recordings.demo.stapel.dev/recordings/api/";
...
/**
 * Provider frame every recordings demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function RecordingsDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createRecordingsRuntime({
      baseUrl: DEMO_BASE,
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <I18nProvider i18n={i18n}>
        <RecordingsProvider runtime={runtime}>{props.children}</RecordingsProvider>
      </I18nProvider>
    </QueryClientProvider>
  );
}
