 ...
 * demo needs: query client, i18n, and the translate runtime.
...
import { createTranslateRuntime } from "../src/index.js";
import { TranslateProvider, registerTranslateI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-translate `/translate/api/v1/`). */
export const DEMO_BASE = "https://translate.demo.stapel.dev/translate/api/v1/";
...
/**
 * Provider frame every translate demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function TranslateDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createTranslateRuntime({
      baseUrl: DEMO_BASE,
      fetch: mockFetch(handlers ?? {}),
    });
    const engine = createI18n({ locale: "en" });
    registerTranslateI18n(engine);
