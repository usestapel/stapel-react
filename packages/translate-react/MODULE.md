...
Headless React flow pair for **stapel-translate**. This is the human companion to the
generated `llms.txt` (agent context) and `manifest.json` (machine catalog).

## Layers

- **api/** — `createTranslateApi(client)`; types are aliases over the
  package-LOCAL generated `components["schemas"]` (`src/api/generated/schema.ts`,
  produced by `pnpm gen:api` from stapel-translate's own `docs/schema.json`; never
  ...
  `api/extensions.ts`.
- **model/** — `translateQueryKeys` (single key factory, `["translate"]`
  namespace), `createTranslateRuntime`, React context/hooks. Declare the
  persist/optimistic policy here as you add read hooks and mutations.
- **flows/** — `toFlowError` + the zero-flow `TRANSLATE_FLOWS` registry shim
  (`registry.ts`, slim wave §21/S3 — `gen:flows` emits no scaffolding for a
  zero-flow module). Once stapel-translate annotates `@flow_step`, `pnpm gen:flows`
...
- **headless/** — render-prop components; `<TranslateProvider>` wires the
  runtime into context. shadcn-copyable (frontend-standard §7).
- **i18n/** — `TRANSLATE_I18N_KEYS` + en bundle; the generated backend error
...
- **demo/** — first-class demos (`defineDemo`, `@stapel/showcase`): `_harness.tsx`
  ...
  starter `Translate.demo.tsx` covers `TranslateProvider`. Demos never ship.
...
2. Alias the stapel-translate schemas you use in `api/types.ts`.
3. Add read hooks + mutations in `model/` and a persist/optimistic policy.
4. Once stapel-translate annotates `@flow_step`, scaffold flow machines from
