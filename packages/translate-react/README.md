...
Headless React flow pair for **stapel-translate** (frontend-standard §2). Business +
...
single component — slim wave §21/S4), one `<TranslateProvider>` for this pair:
...
registerTranslateI18n(i18n); // the pair's key bundle → core's engine
...
Hooks and headless components work anywhere below `<TranslateProvider>`
(`useTranslateApi`, the query/mutation hooks you add in `model/`, the
...
client as a per-module override — `clients={{ translate: runtime.client }}` —
and nest `<TranslateProvider>` next to your other pair providers. The
...
src/
  ...
  headless/   renderless components (TranslateProvider, flow render-props)
...
`./default` is the pair's **shipped** half: `<TranslatePanel/>`, themed through
...
import { registerTranslateI18nRu } from "@stapel/translate-react/i18n/ru";
```
...
| Surface | Path | Gate |
|---|---|---|
| Typed API schema | `src/api/generated/schema.ts`, from stapel-translate's own `docs/schema.json` | `pnpm gen:api:check` |
...
stapel-translate's own `docs/schema.json`; design tokens are tokens-owned
(`pnpm gen:tokens`).
