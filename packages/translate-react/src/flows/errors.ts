 ...
 * module-scoped fallback key (`translate.error.unknown`, an en string in
 * {@link translateI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "translate.error.unknown");
}
