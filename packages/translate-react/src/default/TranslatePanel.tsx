/**
 * `<TranslatePanel/>` — the skin skeleton this pair ships from `./default`.
 *
 ...
 * makes a missing `<TranslateProvider>` a loud error instead of a blank box.
...
export interface TranslatePanelProps extends ThemeModeProp {
  /** Rendered while the screen's own data is in flight. */
  readonly loading?: boolean;
}

export function TranslatePanel(props: TranslatePanelProps): ReactElement {
  const t = useT();
  // Reads the runtime purely so that mounting this panel OUTSIDE
  // <TranslateProvider> throws the context error with the provider's name in
  // it, at the place the mistake was made.
  useTranslateRuntime();

  return (
    <SkinTheme {...(props.mode !== undefined ? { mode: props.mode } : {})}>
      <Card title={t(TRANSLATE_I18N_KEYS.navOverview)} data-testid="translate-panel">
        {props.loading === true ? (
          <Typography.Paragraph>
            {t(TRANSLATE_I18N_KEYS.panelLoading)}
          </Typography.Paragraph>
        ) : (
          <Empty description={t(TRANSLATE_I18N_KEYS.panelEmpty)} />
        )}
      </Card>
    </SkinTheme>
  );
