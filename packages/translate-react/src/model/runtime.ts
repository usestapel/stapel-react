...
/**
 * The wired translate runtime — core's `ModuleRuntime` bound to this pair's
 ...
 * into core's `StapelConfigProvider` (as the default or the `"translate"`
 ...
 */
export type TranslateRuntime = ModuleRuntime<TranslateApi>;

export type CreateTranslateRuntimeOptions = CreateModuleRuntimeOptions;

export function createTranslateRuntime(
  options: CreateTranslateRuntimeOptions
): TranslateRuntime {
  return createModuleRuntime(createTranslateApi, options);
}
