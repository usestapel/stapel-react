...
import {
  webhooksI18nBundleRu,
  registerWebhooksI18nRu,
} from "../src/i18n/ru.js";
import { webhooksI18nBundleEs } from "../src/i18n/es.js";
 ...
 * {@link WEBHOOKS_I18N_KEYS} must have a text in en, ru AND es, and the
 * `{param}` slots must match across all three — a translation that drops a slot
 * renders a sentence with a hole in it.
 */
const BUNDLES = {
  en: webhooksI18nBundleEn,
  ru: webhooksI18nBundleRu,
  es: webhooksI18nBundleEs,
} as const;
...
describe("i18n locale parity", () => {
  for (const [locale, bundle] of Object.entries(BUNDLES)) {
    it(`${locale} covers every key the pair declares`, () => {
      const missing = Object.values(WEBHOOKS_I18N_KEYS).filter(
        (key) => !(key in bundle)
      );
    ...
    it(`${locale} keeps the en {param} slots`, () => {
      for (const key of Object.values(WEBHOOKS_I18N_KEYS)) {
        expect(paramsOf(bundle[key] ?? "").sort(), `${locale}:${key}`).toEqual(
          paramsOf(webhooksI18nBundleEn[key] ?? "").sort()
        );
      }
    });
  }
