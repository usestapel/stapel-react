...
Headless React flow pair for **stapel-webhooks** (frontend-standard §2). Business +
...
```
pnpm add @stapel/webhooks-react @stapel/core @tanstack/react-query react
```

## Wire the app once

One `<StapelProvider>` for the whole app (core's config + query + i18n in a
single component — slim wave §21/S4), one `<WebhooksProvider>` for this pair:
...
} from "@stapel/webhooks-react";

const runtime = createWebhooksRuntime({ baseUrl: "/webhooks/api/v1/" });
const i18n = createI18n({ locale: "en" });
registerWebhooksI18n(i18n); // the pair's key bundle → core's engine

export function Root({ children }: { children: React.ReactNode }) {
  return (
    <StapelProvider client={runtime.client} i18n={i18n} cacheVersion="0.0.0">
      <WebhooksProvider runtime={runtime}>{children}</WebhooksProvider>
    </StapelProvider>
  );
}
```
...
src/
  ...
  headless/   renderless components (WebhooksProvider, flow render-props)
...
stapel-webhooks's own `docs/schema.json`; design tokens are tokens-owned
(`pnpm gen:tokens`).
