/**
 * Shared harness for the webhooks-react demos (frontend-guardrails §4.2). Demos
 ...
 * demo needs: query client, i18n, and the webhooks runtime.
...
import { createWebhooksRuntime } from "../src/index.js";
import { WebhooksProvider, registerWebhooksI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-webhooks `/webhooks/api/v1/`). */
export const DEMO_BASE = "https://webhooks.demo.stapel.dev/webhooks/api/v1/";
...
/**
 * Provider frame every webhooks demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function WebhooksDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createWebhooksRuntime({
      baseUrl: DEMO_BASE,
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <I18nProvider i18n={i18n}>
        <WebhooksProvider runtime={runtime}>
