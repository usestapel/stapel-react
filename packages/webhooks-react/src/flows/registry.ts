/**
 * Zero-flow registry shim (slim wave §21/S3). stapel-webhooks annotates no
 * `@flow_step` yet — its backend `docs/flows.json` carries no `webhooks.*`
 ...
 */
export const WEBHOOKS_FLOWS = {} as const;

/** Canonical flow ids present in flows.json (none yet — see above). */
export type WebhooksFlowId = keyof typeof WEBHOOKS_FLOWS;

export type WebhooksFlowSpec = (typeof WEBHOOKS_FLOWS)[WebhooksFlowId];
