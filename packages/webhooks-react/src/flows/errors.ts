 ...
 * module-scoped fallback key (`webhooks.error.unknown`, an en string in
 * {@link webhooksI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "webhooks.error.unknown");
}
