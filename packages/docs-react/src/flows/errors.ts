 ...
 * module-scoped fallback key (`docs.error.unknown`, an en string in
 * {@link docsI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "docs.error.unknown");
}
