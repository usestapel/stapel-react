 ...
 * module-scoped fallback key (`tasks.error.unknown`, an en string in
 * {@link tasksI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "tasks.error.unknown");
}
