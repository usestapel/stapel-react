/**
 * `<TasksPanel/>` — the skin skeleton this pair ships from `./default`.
 *
 ...
 * makes a missing `<TasksProvider>` a loud error instead of a blank box.
...
export interface TasksPanelProps extends ThemeModeProp {
...
export function TasksPanel(props: TasksPanelProps): ReactElement {
  ...
  // <TasksProvider> throws the context error with the provider's name in
  // it, at the place the mistake was made.
  useTasksRuntime();

  return (
    <SkinTheme {...(props.mode !== undefined ? { mode: props.mode } : {})}>
      <Card title={t(TASKS_I18N_KEYS.navOverview)} data-testid="tasks-panel">
        {props.loading === true ? (
          <Typography.Paragraph>
            {t(TASKS_I18N_KEYS.panelLoading)}
          </Typography.Paragraph>
        ) : (
          <Empty description={t(TASKS_I18N_KEYS.panelEmpty)} />
        )}
      </Card>
    </SkinTheme>
  );
