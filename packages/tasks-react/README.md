...
Headless React flow pair for stapel-tasks: typed API client, TanStack Query hooks, flow machines, headless components, and i18n keys. Zero visual opinion.

Headless React flow pair for **stapel-tasks** (frontend-standard §2). Business +
...
Hooks and headless components work anywhere below `<TasksProvider>`
(`useTasksApi`, the query/mutation hooks you add in `model/`, the
...
client as a per-module override — `clients={{ tasks: runtime.client }}` —
and nest `<TasksProvider>` next to your other pair providers. The
...
src/
  ...
  headless/   renderless components (TasksProvider, flow render-props)
...
`./default` is the pair's **shipped** half: `<TasksPanel/>`, themed through
...
| Surface | Path | Gate |
|---|---|---|
| Typed API schema | `src/api/generated/schema.ts`, from stapel-tasks's own `docs/schema.json` | `pnpm gen:api:check` |
...
stapel-tasks's own `docs/schema.json`; design tokens are tokens-owned
(`pnpm gen:tokens`).
