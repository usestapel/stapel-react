...
- 1e7e718: `<RatingBadge>`'s stars round to the nearest half, never down.
...
- 4b2323b: The rating badge reads number-first, like the reference

  0.8.2 drew the glyphs before the number at every rung. The reference writes
  "4.3 \*", and the owner ruled for that form: score, stars, dot, count — and the
  same order at every rung, so a card that resizes never reshuffles the badge
  under the eye. Only the glyph count and the length of the words change as the
  column narrows.
...
- bbb8454: `RatingBadge` never truncates a word — it drops whole facts instead
...
- cdb9f24: A seller's reviews, not only their number — and the module's own words for its
...
- 94ded40: `<ReviewListPanel emptyState>` — the empty arm is a slot, and `null` is one of its answers
...
- ddaaed6: New hook `useOwnerAggregates(ownerKeys, { targetType? })`, backed by
