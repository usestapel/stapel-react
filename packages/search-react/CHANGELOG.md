...
- f53eff4: A facet group whose codes are a sibling's children is STAGED behind that
...
- f937f8c: `FacetLabels.level` is nullable, like the `vocabulary` it belongs to.
...
- d419615: A facet group fed by SEVERAL dictionaries is still a dictionary.
...
- ba76c1d: The location line's label outranks antd instead of tying it.
...
- f36e9d3: The filter rail builds its choice groups before its ranges, so a counted axis
...
- 7343edb: A range row is dropped when nothing in the answer carries the field
...
- 0e49218: A popular value's count sits inline after the label, not at the column edge
...
- 3deb721: `<SearchPage openFacetGroups>` — a host can name which facet groups open

  The pane opens the first `FACET_OPEN_GROUPS` groups the server counted,
  which is a guess from the evidence it has. The reference opens a different
  set per section and it is not the busiest axes: a flats leaf opens rooms,
  price and area — what somebody narrowing a flat reaches for, rather than
  what the corpus happens to divide on.

  Absent, and every existing caller, is unchanged. Named, exactly that set
  opens. Slugs the answer does not carry are ignored rather than an error — a
  ...
  rail. An empty array is an answer (open nothing), not a fallback to the
  default.

  A group the person has already ticked, and every group while the panel
  search is filtering, still opens regardless: those are facts about the
  session and they outrank any default.

## 0.48.1

### Patch Changes

- 3d97df8: A popular value wraps inside its column instead of over the next one
...
- f6e5f43: The popular-values band's heading is a pair of TABS, and both of them stay on screen.
...
- f0aa7ca: The rail's open dictionary declares the box it actually draws.
...
- 116d12c: The rail FINDS a value instead of opening a dropdown, a chained axis waits for the rung above it and says so, and the makes block's «all of it» link rides with its caption.
  ...
  **The rail's dictionary axis is the box and its values.** `<SearchPage>`'s
  ...
  select-shaped button reading _Any_ with a chevron, which had to be pressed
  ...
  make axis is a box captioned _enter a name_ with the popular values listed
  ...
  eight rows and a _Show all_, with nothing overflowing the rail. `"field"` is
...
- 1b8595c: `toolbarLead` — a host node inside the results toolbar row, at its start.
...
- df257aa: `<SearchResultsPane>`: the results toolbar takes a SECOND line instead of
...
- e0a91d0: A colour facet draws the colour the CATALOGUE knows.
...
- 0552467: The feed pins its sort row, a colour facet shows the colour, and the rail stops
  ...
  **Three verdicts, recorded rather than patched.**
  ...
  _The condition scale is the catalogue's, not this pair's._ The census reads a
  ...
  _The label suffix is an importer defect._ The pair reads the heading the wire
  ...
  _The two empty-rail sentences are already one._ The census found the rail
...
- 3a7321e: search: two things stop moving when the answer lands late (D465, D466), and two levers a host could not reach
...
- 7a46d91: search: the rail is drawn ONCE, in the shape the schema gives it (p41)
...
categoryFeaturesPending>`** — the third state `categoryFeatures` never had. A
    ...
    it _already_ reserves for a load in flight (`data-facets-schema="pending"` on
...
- 723b1bb: search: the rail holds its ground while the next facet answer lands (p43)
...
- 4b057ff: "Search in other categories" no longer mounts late, or grows a row at a time
...
- 117f6ef: Two storefront-integrator gaps, closed.
...
- acb84d1: The ends of a from/to come from the answer that measured them.
...
- 0288214: The rail's attribute-range block reserves its box before the answer fills it.
