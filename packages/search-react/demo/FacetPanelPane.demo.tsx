 ...
 * that silently omits `renderCategoryFilter` looks finished and is not; the
 * dev build draws a named placeholder instead, which is what the first variant
 * photographs. The second shows the same panel narrowed by a shared link: the
...
export default defineDemo({
  ...
  variants: {
    "open-search": {
      description:
        "Nothing narrowed yet: the category and location slots are visible, named holes (dev builds only) rather than a silent absence, and the numeric range row comes from the category schema.",
      viewport: "desktop",
      step: "open",
