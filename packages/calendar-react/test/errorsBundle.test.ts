 ...
 * A hand-edit that drops the generated spread from `calendarI18nBundleEn` fails
 * here.
 */
describe("backend error keys all have an en fallback", () => {
  it("every CALENDAR_ERROR_CODE resolves in calendarI18nBundleEn", () => {
    const missing = CALENDAR_ERROR_CODES.filter(
      (code) => !(code in calendarI18nBundleEn)
    );
    expect(missing).toEqual([]);
  });

  it("the generated fallback bundle covers exactly the registry", () => {
    expect(Object.keys(calendarErrorBundleEn).sort()).toEqual(
      [...CALENDAR_ERROR_CODES].sort()
    );
  });

  it("explains a remediation for every generated code", () => {
    for (const code of CALENDAR_ERROR_CODES) {
      expect(explainCalendarError(code), code).toBeDefined();
    }
  });
});
