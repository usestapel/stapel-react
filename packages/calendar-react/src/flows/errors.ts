 ...
 * module-scoped fallback key (`calendar.error.unknown`, an en string in
 * {@link calendarI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "calendar.error.unknown");
}
