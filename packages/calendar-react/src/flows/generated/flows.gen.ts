...
/**
 * The canonical calendar flow registry — the machine-facing projection of the
 ...
 * `CALENDAR_FLOWS[...].id` so a backend flow rename/re-step surfaces as a drift
 * here (`pnpm gen:flows:check`) rather than a silent analytics/contract skew.
 */
export const CALENDAR_FLOWS = {
...
export function flowEndpoints(id: CalendarFlowId): readonly FlowEndpoint[] {
  ...
  // `CalendarFlowId` = `never` and `CALENDAR_FLOWS` = `{}`, so a direct
  // `CALENDAR_FLOWS[id].steps` does not type-check. Widening to an optional spec
