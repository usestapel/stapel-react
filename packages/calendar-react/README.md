...
Headless React flow pair for stapel-calendar: typed API client, TanStack Query hooks, flow machines, headless components, and i18n keys. Zero visual opinion.

Headless React flow pair for **stapel-calendar** (frontend-standard §2). Business +
...
```
pnpm add @stapel/calendar-react @stapel/core @tanstack/react-query react
```

## Wire the app once

One `<StapelProvider>` for the whole app (core's config + query + i18n in a
single component — slim wave §21/S4), one `<CalendarProvider>` for this pair:
...
Hooks and headless components work anywhere below `<CalendarProvider>`
(`useCalendarApi`, the query/mutation hooks, the render-prop components — see
...
per-module override — `clients={{ calendar: runtime.client }}` — and nest
`<CalendarProvider>` next to your other pair providers. The individual core
...
src/
  ...
  headless/   renderless components (CalendarProvider, flow render-props)
