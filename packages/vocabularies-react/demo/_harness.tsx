/**
 * Shared harness for the vocabularies-react demos (frontend-guardrails §4.2). Demos
 ...
 * demo needs: query client, i18n, and the vocabularies runtime.
...
import { createVocabulariesRuntime } from "../src/index.js";
import { VocabulariesProvider, registerVocabulariesI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-vocabularies `/vocabularies/api/v1/`). */
export const DEMO_BASE = "https://vocabularies.demo.stapel.dev/vocabularies/api/v1/";
...
/**
 * Provider frame every vocabularies demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function VocabulariesDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createVocabulariesRuntime({
      baseUrl: DEMO_BASE,
      fetch: mockFetch(handlers ?? {}),
    });
    const engine = createI18n({ locale: "en" });
    registerVocabulariesI18n(engine);
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <I18nProvider i18n={i18n}>
        <VocabulariesProvider runtime={runtime}>
