...
- c8a1302: The identity disc gets an edge, so it reads as a disc rather than as two letters floating beside a name.
...
- eea3596: `identityTint` / `useIdentityTint` — the disc a person with no photograph wears, decided once for the whole fleet.
...
- 27308a1: **A parameter change is not a first load: `keepPrevious`.**
...
- ff1d884: `SkinNumberField` can state its bound on the element: `min`, `max`, `pattern`
...
- 4d5c5b8: `<PaneGate actionPlacement>`: the pooled footnote can carry the way out of it.

  `action` rendered only in the BLOCKED arm. But a pane a visitor may read and
  not write is `available`, and its per-control gates pool their reasons into
  one footnote — "sign in to reply", "sign in to react", once each — while the
  door out of all of them lived in the one arm that page never renders. Measured
  on a live storefront, where the footnote said what was refused and offered
  nowhere to go.

  `actionPlacement="always"` draws `action` under the pooled sentences too,
  inside the same block, because the reasons and the way out are one statement.
  Only where there is something to pool: no refusal and no reasons means no
  door. The default `"blocked"` is what ships today.

## 0.17.1

### Patch Changes

- 715c236: A chip is named by its label, never by its storage code.
...
- 62901b7: Replace `<Space direction="vertical">` with `<Space orientation="vertical">` (antd 6's non-deprecated prop) in the `skin/permission` and `skin/confirm` footers — silences the antd 6 deprecation warning; spacing and alignment unchanged.
...
- 9587386: A carousel's height stops depending on how many slides it has
...
- 3a1759b: Control SHAPE becomes a host token axis, read live like every colour role.
...
- 45cdc5a: `SkinPickerSheet` gains `onEndReached` — the sheet's half of paging through
