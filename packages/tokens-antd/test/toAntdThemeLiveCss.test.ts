 ...
 * `ramps`, `stapel-tokens` / `pnpm gen:tokens`) — never by touching this
 * published package. The bridge must read that host's LIVE `--stapel-<role>`
 * custom properties at call time, not the compiled-in `colors` snapshot of
 ...
 */
function stubComputedBrand(hex: string | undefined): void {
  vi.stubGlobal(
    "getComputedStyle",
    vi.fn().mockReturnValue({
      getPropertyValue: (prop: string) =>
        prop === "--stapel-brand" && hex !== undefined ? hex : "",
...
describe("toAntdTheme — reads the host's live CSS custom properties", () => {
  ...
  it("prefers a live --stapel-brand over the compiled-in default", () => {
    stubComputedBrand("#1677ff");
