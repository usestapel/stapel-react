/**
 * Shared harness for the moderation-react demos (frontend-guardrails §4.2). Demos
 ...
 * demo needs: query client, i18n, and the moderation runtime.
...
import { createModerationRuntime } from "../src/index.js";
import { ModerationProvider, registerModerationI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-moderation `/moderation/api/v1/`). */
export const DEMO_BASE = "https://moderation.demo.stapel.dev/moderation/api/v1/";
...
/**
 * Provider frame every moderation demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function ModerationDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createModerationRuntime({
      baseUrl: DEMO_BASE,
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <I18nProvider i18n={i18n}>
        <ModerationProvider runtime={runtime}>
