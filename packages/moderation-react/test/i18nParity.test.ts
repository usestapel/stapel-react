...
import {
  moderationI18nBundleRu,
  registerModerationI18nRu,
} from "../src/i18n/ru.js";
 ...
 * {@link MODERATION_I18N_KEYS} must have a text in en, ru AND es, and the
...
describe("i18n locale parity", () => {
  for (const [locale, bundle] of Object.entries(BUNDLES)) {
    it(`${locale} covers every key the pair declares`, () => {
      const missing = Object.values(MODERATION_I18N_KEYS).filter(
        (key) => !(key in bundle)
      );
    ...
    it(`${locale} keeps the en {param} slots`, () => {
      for (const key of Object.values(MODERATION_I18N_KEYS)) {
        expect(paramsOf(bundle[key] ?? "").sort(), `${locale}:${key}`).toEqual(
          paramsOf(moderationI18nBundleEn[key] ?? "").sort()
        );
      }
    });
  }
