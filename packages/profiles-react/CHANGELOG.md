...
- 1e7e718: `<PersonRow>`'s name is allowed to give way, so its ellipsis can fire.
...
- 6cdbd83: The tenure line says "since", in Russian too — the month is declined through `Intl`'s own data.
...
- 97232e6: A person with no photograph gets their initials on a deterministic tinted disc, legible in both themes.
...
- df257aa: `<PersonRow trailing>`: the qualifier slot may SHRINK.
...
- ae52969: A seller's phone number: managed by its owner, handed over on purpose.
...
- 2de3357: The seller page is a person with sections, not one filtered list.
...
- 7187a61: Tenure on the public read, a face outside the row, and a name that is a heading.
...
- ab0e7a7: profiles: a person's name as a page heading, with the heading's height reserved while the read is in flight (D453)
...
- ad32b78: `<PersonRow size="compact">` — the same identity in the space of a caption
...
- aaf675f: `<PersonRow>` takes `href` and `linkComponent`: a person's name can be a real
  link.
...
- 35dde9f: `seller_type` on the public profile — regenerated against stapel-profiles 0.19.0.
...
- e2e932d: fix(profiles-react): an uploaded avatar carries its own provenance
  ...
  Nobody remembered. On a client sandbox **2 of 2** profiles that ever had
...
- f2654cf: InitialSetupPrompt canon (workspaces-org-program §B5) — a client onboarding modal ported into the pair as the display-name/first-run prompt every host reuses:
  ...
  - i18n: new `profiles.initialSetup.*` keys (title/subtitle/name_placeholder/save/saving/skip), en + ru, texts ported from a client app.
