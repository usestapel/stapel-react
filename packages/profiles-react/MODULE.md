# @stapel/profiles-react — module guide

Headless React flow pair for **stapel-profiles**. This is the human companion to the
generated `llms.txt` (agent context) and `manifest.json` (machine catalog).

## Layers

- **api/** — `createProfilesApi(client)`; types are aliases over the generated
...
- **model/** — `profilesQueryKeys` (single key factory, `["profiles"]`
  namespace), `createProfilesRuntime`, React context/hooks. Declare the
...
- **flows/** — `createFlowMachine`-based machines (primitive imported from
  `@stapel/core`), bound to the generated `PROFILES_FLOWS` registry. Scaffold
  new machines from flows.json; keep them under `gen:flows:check`.
- **headless/** — render-prop components; `<ProfilesProvider>` wires the
  runtime into context. shadcn-copyable (frontend-standard §7).
- **i18n/** — `PROFILES_I18N_KEYS` + en bundle; the generated backend error
  ...
  polish/UI copy → pair locale bundle (`@stapel/profiles-react/i18n/ru`:
  ...
  (`@stapel/profiles-react/i18n/es`) is the same contour with a DECLARED partial
...
- **demo/** — first-class demos (`defineDemo`, `@stapel/showcase`): `_harness.tsx`
  ...
  starter `Profiles.demo.tsx` covers `ProfilesProvider`. Demos never ship.

## Extension seams (frontend-standard §7)

- Client is injected via `<ProfilesProvider>` / core's `StapelConfigProvider`
...
2. Alias the stapel-profiles schemas you use in `api/types.ts`.
3. Add read hooks + mutations in `model/` and a persist/optimistic policy.
4. Once stapel-profiles annotates `@flow_step`, scaffold flow machines from
