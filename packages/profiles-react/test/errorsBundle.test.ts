...
import { profilesI18nBundleEn } from "../src/i18n/keys.js";
import {
 ...
 * A hand-edit that drops the generated spread from `profilesI18nBundleEn` fails
 * here.
 */
describe("backend error keys all have an en fallback", () => {
  it("every PROFILES_ERROR_CODE resolves in profilesI18nBundleEn", () => {
    const missing = PROFILES_ERROR_CODES.filter(
      (code) => !(code in profilesI18nBundleEn)
    );
    expect(missing).toEqual([]);
  });

  it("the generated fallback bundle covers exactly the registry", () => {
    expect(Object.keys(profilesErrorBundleEn).sort()).toEqual(
  ...
  it("explains a remediation for every generated code", () => {
    for (const code of PROFILES_ERROR_CODES) {
      expect(explainProfilesError(code), code).toBeDefined();
    }
  });
});
