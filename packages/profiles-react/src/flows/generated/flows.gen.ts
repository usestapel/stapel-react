...
/**
 * The canonical profiles flow registry — the machine-facing projection of the
 ...
 * `PROFILES_FLOWS[...].id` so a backend flow rename/re-step surfaces as a drift
 * here (`pnpm gen:flows:check`) rather than a silent analytics/contract skew.
 */
export const PROFILES_FLOWS = {
...
export function flowEndpoints(id: ProfilesFlowId): readonly FlowEndpoint[] {
  ...
  // `ProfilesFlowId` = `never` and `PROFILES_FLOWS` = `{}`, so a direct
  // `PROFILES_FLOWS[id].steps` does not type-check. Widening to an optional spec
