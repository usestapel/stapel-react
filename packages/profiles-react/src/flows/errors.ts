 ...
 * module-scoped fallback key (`profiles.error.unknown`, an en string in
 * {@link profilesI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "profiles.error.unknown");
}
