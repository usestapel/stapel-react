...
/**
 * The wired profiles runtime — core's `ModuleRuntime` bound to this pair's
 ...
 * into core's `StapelConfigProvider` (as the default or the `"profiles"`
 ...
 */
export type ProfilesRuntime = ModuleRuntime<ProfilesApi>;

export type CreateProfilesRuntimeOptions = CreateModuleRuntimeOptions;

export function createProfilesRuntime(
  options: CreateProfilesRuntimeOptions
): ProfilesRuntime {
  return createModuleRuntime(createProfilesApi, options);
}
