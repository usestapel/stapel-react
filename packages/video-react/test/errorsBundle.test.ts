...
import { videoI18nBundleEn } from "../src/i18n/keys.js";
import {
  VIDEO_ERROR_CODES,
  videoErrorBundleEn,
  explainVideoError,
 ...
 * A hand-edit that drops the generated spread from `videoI18nBundleEn` fails
 * here.
 */
describe("backend error keys all have an en fallback", () => {
  it("every VIDEO_ERROR_CODE resolves in videoI18nBundleEn", () => {
    const missing = VIDEO_ERROR_CODES.filter(
      (code) => !(code in videoI18nBundleEn)
    );
    expect(missing).toEqual([]);
  });

  it("the generated fallback bundle covers exactly the registry", () => {
    expect(Object.keys(videoErrorBundleEn).sort()).toEqual(
      [...VIDEO_ERROR_CODES].sort()
    );
  });

  it("explains a remediation for every generated code", () => {
    for (const code of VIDEO_ERROR_CODES) {
      expect(explainVideoError(code), code).toBeDefined();
    }
  });
});
