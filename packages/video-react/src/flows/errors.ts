 ...
 * module-scoped fallback key (`video.error.unknown`, an en string in
 * {@link videoI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "video.error.unknown");
}
