/**
 * Zero-flow registry shim (slim wave §21/S3). stapel-video annotates no
 * `@flow_step` yet — its backend `docs/flows.json` carries no `video.*`
 ...
 */
export const VIDEO_FLOWS = {} as const;

/** Canonical flow ids present in flows.json (none yet — see above). */
export type VideoFlowId = keyof typeof VIDEO_FLOWS;

export type VideoFlowSpec = (typeof VIDEO_FLOWS)[VideoFlowId];
