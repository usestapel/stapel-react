 ...
 * and the table follows with zero code — a far better deal than a
...
export interface VideoSkinThemeProps {
  /**
...
/**
 * `<VideoSkinTheme/>` — the self-theming wrapper of
 * `@stapel/video-react/default`. Standalone use is supported (wrap any
 * composition of the skin's parts once); the shipped surfaces
 * (`ScopeUsagePane`, `ScopeUsageTable`) each already wrap themselves, and
 * nested antd `ConfigProvider`s merge, so composing them under one
 * `VideoSkinTheme` stays correct.
 */
export function VideoSkinTheme(props: VideoSkinThemeProps): ReactElement {
  ...
  return (
    <ConfigProvider theme={theme}>
      <div
        data-video-skin-root
        data-video-skin-mode={mode}
        style={{
