/**
 * Shared harness for the video-react demos (frontend-guardrails §4.2). Demos
 ...
 * headless component needs: query client, i18n, and the video runtime.
 */
...
import { VideoProvider, registerVideoI18n } from "../src/index.js";

/** The base every mock handler mounts on (mirrors stapel-video `/video/api/v1/`). */
export const DEMO_BASE = "https://video.demo.stapel.dev/video/api/v1/";
...
/**
 * Provider frame every video demo variant renders inside. Builds a fresh mock
 * runtime + query client per mount so variants stay isolated.
 */
export function VideoDemoHarness(props: {
  handlers?: DemoHandlers;
  children: ReactNode;
}): ReactElement {
  const { handlers } = props;
  const { runtime, queryClient, i18n } = useMemo(() => {
    const rt = createVideoRuntime({
      baseUrl: DEMO_BASE,
