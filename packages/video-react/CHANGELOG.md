...
- 1f8df08: Regenerated against **stapel-video 0.12.0**: the pin was one minor behind
  (0.11.2). The declared backend contract moves to `>=0.12 <0.13` in
  `manifest.json` and `llms.txt`. `docs/schema.json`, `docs/errors.json` and
  `docs/flows.json` are byte-identical across 0.11.2..0.12.0 — a pure
  freshness bump, nothing generated in this package moves beyond the version
  stamp.
...
- 67d79c2: `NARROW_LIMIT` reads the `tablet` token instead of restating it.
...
- 70cee25: The call surface is drawn from the theme in BOTH modes — the dark theme no longer gets a white sheet.
...
- 06a70ce: The mute-toggle test waits for the pressed VALUE, not just for the room to
  have been called.
