...
- 8322284: Regenerated against **stapel-categories 0.23.1**: drf-spectacular's
...
- 1a1c240: `<CategoryMegaMenu minWidth>` defaults to the tokens' `desktop` rung, not a
  hard-coded 1024.
...
- 9d05783: The mega-menu is an overlay in its own right: its own scroll box, the page held still, a wider rail, and a hover
...
- e426b2d: A tile with no art of its own can inherit its parent's
...
- 1292589: The compact row keeps its name, and the scroller's column count is a prop
...
- df257aa: `<CategoryTileGrid>`: a tile caption breaks between WORDS, never inside one.
...
- 0992fab: A breadcrumb trail is not a panel.
...
- 96cdb9b: A flat tile grid has no panel behind it.
  ...
  - `surface="bare"` paints _nothing_, including the text colour `"raised"` would
...
- f71cdcc: A compact tile without a fill is a list row.
...
- 97c14d9: The flat tile is a stack, and the tile stage reserves the rows it draws.
...
- 552cca2: A tile is an icon and a name, and one token spaces every block on a catalogue
  page.
...
- 68d5906: The Spanish refusals quote the slug, like the Russian ones already did.
...
- 70ca317: The nine refusals this pair used to write are the module's own words now
...
- b7dd445: categories: the catalogue says which feature of a category is the make
...
- 9c9316d: categories: the breadcrumb trail belongs to the frame, not to the address bar
...
- 27308a1: **`<CategoryPage>` holds the page it is showing until the next one is whole.**
...
- ba42588: **A tile label's line count is a number, not an `!important`.**
...
- 9b489ef: `<CategoryPage gutter={false}>` — a page inside a shell must not indent itself
  twice.
...
- 83b1caf: `tn_children_pks` is no longer the answer to "does this row have children" —
...
- a652ca8: Two additions, one round.
...
- a7b8190: A transparent wrapper's breadcrumb crumb no longer links anywhere.
...
- 5e4ee0f: A one-rung import wrapper is now invisible to browsing — the census
