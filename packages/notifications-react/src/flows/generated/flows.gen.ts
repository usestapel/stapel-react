...
/**
 * The canonical notifications flow registry — the machine-facing projection of the
 ...
 * `NOTIFICATIONS_FLOWS[...].id` so a backend flow rename/re-step surfaces as a drift
 * here (`pnpm gen:flows:check`) rather than a silent analytics/contract skew.
 */
export const NOTIFICATIONS_FLOWS = {
...
export function flowEndpoints(id: NotificationsFlowId): readonly FlowEndpoint[] {
  ...
  // `NotificationsFlowId` = `never` and `NOTIFICATIONS_FLOWS` = `{}`, so a direct
  // `NOTIFICATIONS_FLOWS[id].steps` does not type-check. Widening to an optional spec
