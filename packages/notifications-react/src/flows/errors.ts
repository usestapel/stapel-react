 ...
 * module-scoped fallback key (`notifications.error.unknown`, an en string in
 * {@link notificationsI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "notifications.error.unknown");
}
