/**
 * Zero-flow registry shim (slim wave §21/S3). stapel-notifications annotates no
 * `@flow_step` yet — its backend `docs/flows.json` carries no `notifications.*`
 ...
 */
export const NOTIFICATIONS_FLOWS = {} as const;

/** Canonical flow ids present in flows.json (none yet — see above). */
export type NotificationsFlowId = keyof typeof NOTIFICATIONS_FLOWS;

export type NotificationsFlowSpec = (typeof NOTIFICATIONS_FLOWS)[NotificationsFlowId];
...
export function flowEndpoints(id: NotificationsFlowId): readonly FlowEndpoint[] {
  // Same widened body as the generated registry's — valid for the zero-flow
  // shape AND correct once flows exist.
  const spec = NOTIFICATIONS_FLOWS[id] as
