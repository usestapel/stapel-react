...
import { NOTIFICATIONS_ERRORS, NOTIFICATIONS_ERROR_CODES } from "../src/i18n/errorsMap.js";
import {
...
import {
  ...
  notificationsI18nBundleRu,
  registerNotificationsI18nRu,
} from "../src/i18n/ru.js";
...
describe("generated ru error bundle", () => {
  it("covers exactly the backend registry (every code, nothing else)", () => {
    expect(Object.keys(notificationsErrorBundleRu).sort()).toEqual(
  ...
  it("every ru text preserves the canon's {param} slots", () => {
    for (const code of NOTIFICATIONS_ERROR_CODES) {
      expect(paramsOf(notificationsErrorBundleRu[code]).sort(), code).toEqual(
        [...NOTIFICATIONS_ERRORS[code].params].sort()
      );
    }
  });
  ...
  it("param-bearing keys interpolate in ru", () => {
    ...
    registerNotificationsI18nRu(i18n);
...
describe("ru bundle covers the pair's UI keys", () => {
  it("every NOTIFICATIONS_I18N_KEYS value has a ru text", () => {
    const missing = Object.values(NOTIFICATIONS_I18N_KEYS).filter(
      (key) => !(key in notificationsI18nBundleRu)
    );
  ...
  it("ui {param} slots match the en copy", () => {
    for (const key of Object.values(NOTIFICATIONS_I18N_KEYS)) {
      expect(paramsOf(notificationsI18nBundleRu[key] ?? "").sort(), key).toEqual(
        paramsOf(notificationsI18nBundleEn[key] ?? "").sort()
      );
    }
  });
});
...
describe("locale switching through the core engine", () => {
  it("setLocale('ru') switches texts; back to en restores them", async () => {
    ...
    registerNotificationsI18n(i18n); // en floor + polish under "en"
    registerNotificationsI18nRu(i18n); // ru locale from the subpath

    const enBefore = i18n.t("notifications.error.unknown");
    expect(enBefore).toBe(notificationsI18nBundleEn["notifications.error.unknown"]);
  ...
  it("a missing ru key degrades to its ENGLISH text, never to a raw key", async () => {
    ...
    const { "notifications.feed.end": _dropped, ...partialRu } = notificationsI18nBundleRu;
    const i18n = createI18n({ locale: "en" });
    i18n.registerBundle("ru", notificationsI18nBundleEn);
    i18n.registerBundle("ru", partialRu);
    await i18n.setLocale("ru");
    expect(i18n.t("notifications.feed.end")).toBe(
      notificationsI18nBundleEn["notifications.feed.end"]
    );
  });
  ...
  it("a host bundle registered AFTER the pair's ru wins (override without a fork)", async () => {
    const i18n = createI18n({ locale: "en" });
    registerNotificationsI18nRu(i18n);
    i18n.registerBundle("ru", { "notifications.error.unknown": "Ошибка (кастом)" });
    await i18n.setLocale("ru");
    expect(i18n.t("notifications.error.unknown")).toBe("Ошибка (кастом)");
  });
});
...
describe("tree-shake purity: ru is NOT in the main entry", () => {
  it("dist/index.js module graph never reaches the ru modules", () => {
    ...
    // pulls for `import ... from "@stapel/notifications-react"`). The
    // size-limit gate (main entry byte-budget unchanged by the locale) is the
    // second lock.
