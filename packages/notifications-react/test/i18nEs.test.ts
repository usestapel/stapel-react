...
import type { NotificationsErrorCode } from "../src/i18n/errorsMap.js";
import {
...
import {
  ...
  notificationsI18nBundleEs,
  registerNotificationsI18nEs,
} from "../src/i18n/es.js";
...
function codeWithParams(): NotificationsErrorCode {
  for (const code of NOTIFICATIONS_ERROR_CODES) {
    if (NOTIFICATIONS_ERRORS[code].params.length > 0) return code;
  }
...
function codeWithoutParams(): NotificationsErrorCode {
  for (const code of NOTIFICATIONS_ERROR_CODES) {
    if (NOTIFICATIONS_ERRORS[code].params.length === 0) return code;
  }
...
describe("generated es error bundle", () => {
  it("covers exactly the backend registry (every code, nothing else)", () => {
    expect(Object.keys(notificationsErrorBundleEs).sort()).toEqual(
  ...
  it("every es text preserves the canon's {param} slots", () => {
    for (const code of NOTIFICATIONS_ERROR_CODES) {
      expect(paramsOf(notificationsErrorBundleEs[code]).sort(), code).toEqual(
  ...
  it("param-bearing keys interpolate in es", async () => {
    ...
    const params = Object.fromEntries(
      NOTIFICATIONS_ERRORS[code].params.map((name, i) => [name, `v${i}`])
    );
    const i18n = createI18n({ locale: "en" });
    registerNotificationsI18nEs(i18n);
    await i18n.setLocale("es");
    const text = i18n.t(code, params);
    for (const name of NOTIFICATIONS_ERRORS[code].params) {
...
describe("locale switching through the core engine", () => {
  it("setLocale('es') switches error texts; back to en restores them", async () => {
    ...
    registerNotificationsI18nEs(i18n); // es locale from the subpath

    expect(i18n.t(code)).toBe(notificationsI18nBundleEn[code] ?? i18n.t(code));
    await i18n.setLocale("es");
    expect(i18n.t(code)).toBe(notificationsErrorBundleEs[code]);
    await i18n.setLocale("en");
    expect(i18n.t(code)).not.toBe(notificationsErrorBundleEs[code]);
  });
