...
Headless React flow pair for **stapel-notifications** (frontend-standard §2). Business +
...
single component — slim wave §21/S4), one `<NotificationsProvider>` for this pair:
...
(`useNotificationsApi`, the query/mutation hooks, the render-prop components — see
...
per-module override — `clients={{ notifications: runtime.client }}` — and nest
...
difference stated up front: **it translates the 43 backend error codes, not
the pair's own UI copy.** `registerNotificationsI18nEs` registers the en floor UNDER the
Spanish texts, so a Spanish-speaking user reads Spanish error messages and
English UI copy — never a raw key.

```tsx
import { registerNotificationsI18nEs } from "@stapel/notifications-react/i18n/es";

registerNotificationsI18n(i18n);      // en floor + polish
registerNotificationsI18nEs(i18n);    // es locale (generated from the backend catalog)
await i18n.setLocale("es");    // live switch; untranslated UI keys read English
```

Error texts are generated from stapel-notifications's `translations/errors.es.json`
...
additively — this subpath and `notificationsI18nBundleEs` keep their names.
