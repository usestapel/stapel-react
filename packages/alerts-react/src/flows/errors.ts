 ...
 * module-scoped fallback key (`alerts.error.unknown`, an en string in
 * {@link alertsI18nBundleEn}) so a non-`StapelApiError` fault still renders
...
export function toFlowError(error: unknown): FlowError {
  return coreToFlowError(error, "alerts.error.unknown");
}
