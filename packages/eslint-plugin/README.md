...
| `stapel/no-self-measuring-ref` | a measuring hook's ref (`useNarrow`, `useElementWidth`, `useSplitLayout`, `useImageSlot`, `useElementBox`, `useRailEdges`, `useRailFits`, … — both call shapes: the hook hands back a ref, or the hook takes one) attached to an element whose OWN WIDTH that same measurement decides, in the same component. Three shapes: a width-affecting style property read off the answer; a whole `style={measured ? A : B}` whose two object literals differ in a width-affecting property; a `className` chosen by the answer; or the element being rendered only inside a branch taken on it. A measurement that feeds itself has no fixed point — 1440 says card, the card is 360, 360 says full-screen, full-screen is 1440 — so the layout flips every animation frame and React rebuilds the subtree, which makes every control inside **unclickable** (a press and a release land on two different DOM nodes, so no `click` is ever dispatched). That is video-react 0.3.6's `<IncomingCallOverlay>`: desktop calls never connected, while 390 worked because the phone happened to sit on the stable arm. "Width-affecting" is an **allowlist**, not a denylist — `aspectRatio` on a `width: 100%` frame moves the height only, which is `CallPanel`'s legitimate shape, and a denylist would have flagged it and got the rule switched off. The `className` case is the one deliberate over-approximation (the CSS is not visible from here). **`warn` in `recommended` pending the fleet-wide sweep, `error` in `strict`.** Tune with `hooks` / `hooksOverride`. Cross-file refs, constant classes whose CSS varies, shrink-to-fit boxes and opaque `style={f(x)}` are out of reach and said so in [the rule's docs page](docs/rules/no-self-measuring-ref.md) |
| `stapel/no-skin-color-literal` | a hex / `rgb()` / `hsl()` / named colour written down inside `src/default/**` — the skin layer. Four surfaces: a **style object** (`style={…}`, any `*Style`/`styles` attribute or property, a `CSSProperties`-annotated constant), a `css`/`styled` **tagged template**, a **string that IS css** (it carries `prop: value` declaration syntax — `cardGallery.ts`/`detailGallery.ts` build their stylesheets by concatenation, which no `css`-tag rule can see), and a **colour-named JSX attribute** (`color`, `bgColor`, `fill`, `stroke`, `*Color` — where `QrCanvas.tsx`'s `color="#000000" bgColor="#ffffff"` sat through `no-raw-colors` at `error` the whole time). Inside a style object the key does not gate hex, so `filter: "drop-shadow(0 0 2px #000)"` is caught too; identifiers resolve through ONE same-file `const` hop, so a tidy name does not launder the value. A NAMED colour counts only as the whole value of a colour property. antd's thirteen **preset palette keys are exempt on a JSX colour prop only** — `<Tag color="green">` is generated from the active theme's seed and moves with the mode; `white`/`black`/`gray`/`silver` are exempt nowhere. Allowed everywhere: `transparent`, `currentColor`, `inherit` and the CSS-wide keywords. A deliberate, theme-independent colour is **marked, not disabled** — `// stapel-color-literal: <why>` on the enclosing statement covers everything inside it and must carry a reason (a reasonless marker is itself reported). `@stapel/video-react` 0.3.6 drew the in-call screen at **1.09:1** under the dark theme and shipped its fix with a grep of its own skin; this is that grep for the other twenty-eight packages. **Read the rule header before trusting a green run**: the 0.3.6 defect was a `token.*` read ABOVE the component's `<SkinTheme>`, with no literal anywhere — that half is `no-local-skin-theme` / `no-hardcoded-theme-mode` plus a render-time test. **`warn` in `recommended` (six live hits, all already justified in prose), `error` in `strict`.** Tune with `include`. Limits in [the rule's docs page](docs/rules/no-skin-color-literal.md) |
| `stapel/no-pinned-corpus-fact` | a probe or end-to-end walker pinning a fact about somebody else's corpus. Three shapes: an **equality** between a count-shaped expression (`.length`/`.size`, or a name ending `count`/`total`/`results`/`items`/`rows`/`hits`) and a numeric literal ≥ `maxStructuralCount` (10) — including `expect(x).toBe(n)` and `expect(x).toHaveLength(n)`; an **id-shaped literal as a whole path segment** of a url or as an `*id*=` query value (three or more digits, or a uuid); and an **env fallback to an id** (`process.env.X \|\| "1345"`) whose binding is interpolated into a url in the same file. Relational bounds, `=== 0` and `=== 1` are silent on purpose — a bound survives a reseed, and "cleanup left nothing" is a claim about behaviour; `?limit=500`, `?page=2`, a 1440 viewport and a 60000 timeout never sit between two slashes and are invisible. A filesystem path (a scratch dir named with a uuid) and a **sentinel** (`/l/9999999`, a uuid of one repeated character per group — written down precisely so the service can answer "no such thing") are carved out. One probe asserted `finalCount === 390` and began failing on reseeds where cleanup had worked perfectly; another pinned a listing id a reseed deleted, and because a SPA answers **200 for any `/l/<id>`** it walked on through every assertion that followed. Scoped IN THE RULE to `deploy/probes/**`, `**/probes/**`, `**/e2e/**`, `**/walkers/**`, `*.probe.*`, `*.e2e.*` — never unit tests, whose corpus is their own fixture. `error` in `recommended`. Tune with `include` / `maxStructuralCount`. Limits in [the rule's docs page](docs/rules/no-pinned-corpus-fact.md) |
| `stapel/no-inert-typography-override` | a **typography declaration in a package's own stylesheet at a specificity antd beats** — the rule ships present and inert. antd generates a per-theme class for Typography (`.css-wgezi7`), carrying its own font ladder, and injects it into `<head>` at RUNTIME, **after** a package's static sheet: a single class or a single `[data-testid]` is (0,1,0), the same score, so **order** decides and antd's is second; on a heading antd's `h1:where(.css-x).ant-typography` is (0,1,1) and wins **outright**. `@container` / `@media` wrappers add **no** specificity, so the rule inside one is graded as if it were flat. The bar is (0,2,0) — two class-level components, which is what doubling the last compound (`.price.price`) buys — and `!important` is never reported. Four instances in one wave: `listings-react` 0.36.0's card price and title (measured live: class on the element, `@container` rule in the sheet, computed size still antd's 16px), 0.30.5's title clamp against a HOST sheet's `display:block` (60 titles, `-webkit-line-clamp` in the computed style, nothing clamped), and the storefront's category H1. Graded on the **selector shape that ships**, never on the expression that builds it: `.${clampClass(lines)}.${clampClass(lines)}` reads as (0,2,0) whatever the function returns — the per-package test this replaces asserted the literal template and broke on a refactor that changed no emitted byte. The ladder is short on purpose (`font-*`, `line-height`, `letter-spacing`, `word-spacing`, `text-transform`, `text-overflow`, the clamp trio): `color`/`text-decoration` are OUT because a link wrapper's `color:inherit` is correct code, and listing them would have reported the very commit that fixed the defect. **`warn` in `recommended` (four live hits, one of them real), `error` in `strict`.** Tune with `properties` / `propertiesOverride`. Which element the class lands on, a host's own stylesheet, and a sheet whose braces do not balance in one expression are all out of reach — said so in [the rule's docs page](docs/rules/no-inert-typography-override.md) |
...
| `stapel/no-hardcoded-theme-mode` | `const { mode = "light" } = props`, `props.mode ?? "dark"`, `toAntdThemeConfig("light")`, **and `resolveThemeMode()`** inside `src/default/**` — plus, in **every** source file (0.13.0), a literal `mode`/`themeMode`/`colorMode` JSX attribute: `<StorefrontShell mode="light"/>` is the same rendered defect at the call site, and it is where it actually shipped (a storefront with dark tokens in both brand files and no way to reach them, because the host pinned the shell). antd's own `theme="dark"` prop is deliberately NOT covered — same word, vendor meaning. Demos (`**/demo/**`, `*.demo.*`), stories and test paths are exempt in the RULE, not only in the preset: pinning both sides is what those files are for. Tune with `jsxModeAttributes`. Three such lines meant the auth skin rendered light inputs and an invisible heading under `<html data-theme="dark">` (CF-1, reproduced with the attribute set before first render). The document owns the mode: `useThemeMode()` / `<SkinTheme>` from `@stapel/tokens-antd/skin`. The last of the four is the second half of the same defect and reported separately (`staleModeRead`): `resolveThemeMode()` SAMPLES the document once per render, so a host that flips `data-theme` at runtime (shell-react ships that control) leaves already-mounted skins on the old side — which is why the showcase's dark toggle is a paper feature. `useThemeMode()` is the same answer through `useSyncExternalStore` over a `MutationObserver`, i.e. a subscription. Tune with `staleModeFunctions` |
